...
local function LoadConfiguration(Configuration)
	...
	for FlagName, FlagValue in next, Data do
		if RayfieldLibrary.Flags[FlagName] then
			...
			RayfieldLibrary:Notify({Title = "Flag Error", Content = "Rayfield was unable to find '"..tostring(FlagName or "nil").. "''" in the current script"})
		end
	end
end
...
function Hide()
	...
	for _, tabbtn in ipairs(TabList:GetChildren()) do
		if tabbtn.ClassName == "Frame" and tabbtn.Name ~= "Placeholder" then
			...
			if tabbtn:FindFirstChild("Shadow") then TweenService:Create(tabbtn.Shadow, TweenInfo.new(0.3, Enum.EasingStyle.Quint), {ImageTransparency = 1}):Play() end
	...
	for _, tabbtn in ipairs(TabList:GetChildren()) do
		if tabbtn.ClassName == "Frame" and tabbtn.Name ~= "Placeholder" then
			if tostring(Elements.UIPageLayout.CurrentPage) == tabbtn.Title.Text then
				...
				if tabbtn:FindFirstChild("Shadow") then TweenService:Create(tabbtn.Shadow, TweenInfo.new(0.3, Enum.EasingStyle.Quint), {ImageTransparency = 0.9}):Play() end
				...
				if tabbtn:FindFirstChild("Shadow") then TweenService:Create(tabbtn.Shadow, TweenInfo.new(0.3, Enum.EasingStyle.Quint), {ImageTransparency = 0.7}):Play() end
...
function RayfieldLibrary:CreateWindow(Settings)
	...
	pcall(function()
		...
		CFileName = tostring((Settings.ConfigurationSaving and Settings.ConfigurationSaving.FileName) or "config")
	...
	for _, TabButton in ipairs(TabList:GetChildren()) do
		if TabButton.ClassName == "Frame" and TabButton.Name ~= "Placeholder" then
			...
			if TabButton:FindFirstChild("Shadow") then TabButton.Shadow.ImageTransparency = 1 end
	...
	-- Ensure a Shadow child exists to avoid nil errors on templates without it
	local __shadow = TabButton:FindFirstChild("Shadow")
	if not __shadow then
		__shadow = Instance.new("ImageLabel")
		__shadow.Name = "Shadow"
		__shadow.BackgroundTransparency = 1
		__shadow.ImageTransparency = 1
		__shadow.Size = UDim2.new(1, 0, 1, 0)
		__shadow.Position = UDim2.new(0, 0, 0, 0)
		__shadow.Parent = TabButton
	end
		...
		if Image then
			...
			TabButton.Image.Image = "rbxassetid://"..tostring(Image or "")
		...
		if SelectedTheme ~= RayfieldLibrary.Theme.Default then
			if TabButton:FindFirstChild("Shadow") then TabButton.Shadow.Visible = false end
		end
		...
		if FirstTab then
			...
			if TabButton:FindFirstChild("Shadow") then TweenService:Create(TabButton.Shadow, TweenInfo.new(0.3, Enum.EasingStyle.Quint), {ImageTransparency = 0.7}):Play() end
		else
			FirstTab = Name
			TabButton.BackgroundColor3 = SelectedTheme.TabBackgroundSelected
			TabButton.Image.ImageColor3 = SelectedTheme.SelectedTabTextColor
			TabButton.Title.TextColor3 = SelectedTheme.SelectedTabTextColor
			if TabButton:FindFirstChild("Shadow") then TweenService:Create(TabButton.Shadow, TweenInfo.new(0.3, Enum.EasingStyle.Quint), {ImageTransparency = 0.9}):Play() end
        ...
        -- ===== Inventory Grid =====
        function Tab:CreateInventoryGrid(GridSettings)
            -- GridSettings defaults
            GridSettings = GridSettings or {}
            GridSettings.Name        = GridSettings.Name or "Inventory"
            GridSettings.Items       = GridSettings.Items or {} -- { {id,name,icon,qty,rarity,equipped}, ... }
            GridSettings.CellSize    = GridSettings.CellSize or UDim2.new(0, 84, 0, 84)
            GridSettings.CellPadding = GridSettings.CellPadding or UDim2.new(0, 8, 0, 8)
            GridSettings.ShowSearch  = (GridSettings.ShowSearch ~= false)
            GridSettings.ShowStats   = (GridSettings.ShowStats ~= false)
            GridSettings.OnClick     = GridSettings.OnClick or function(_) end
            GridSettings.OnRightClick= GridSettings.OnRightClick or function(_) end
            GridSettings.OnHover     = GridSettings.OnHover or function(_,_) end
            GridSettings.Sorter      = GridSettings.Sorter or function(a,b)
                -- เรียง: equipped > rarity > qty > name
            ...
            -- Root element (ใช้สไตล์ของ Element ปกติ)
            local Root = Elements.Template.Paragraph:Clone()
            Root.Name = GridSettings.Name
            Root.Title.Text = GridSettings.Name
            Root.Content.TextTransparency = 1 -- ซ่อนไว้ เราจะใส่คอนเทนต์เอง
            Root.Visible = true
            Root.Parent = TabPage

            -- สี/สไตล์ตามธีม
            Root.BackgroundColor3 = SelectedTheme.ElementBackground
            Root.UIStroke.Color   = SelectedTheme.ElementStroke
            ...
            Header.Position = UDim2.new(0,10, 0, 36)

            -- Search box
            local SearchFrame = Instance.new("Frame")
            SearchFrame.Name = "Search"
            SearchFrame.Parent = Header
            SearchFrame.Size = UDim2.new(0, 220, 1, 0)
            SearchFrame.BackgroundColor3 = SelectedTheme.InputBackground
            SearchFrame.BackgroundTransparency = GridSettings.ShowSearch and 0 or 1
            SearchFrame.Visible = GridSettings.ShowSearch
            local sUIC = Instance.new("UICorner", SearchFrame); sUIC.CornerRadius = UDim.new(0, 6)
            local sStroke = Instance.new("UIStroke", SearchFrame); sStroke.Color = SelectedTheme.InputStroke; sStroke.Thickness = 1
            ...
            SearchBox.Font = Enum.Font[SelectedTheme.TextFont] or Enum.Font.Gotham
            SearchBox.TextXAlignment = Enum.TextXAlignment.Left
            SearchBox.PlaceholderText = "ค้นหาไอเท็ม (ชื่อ/แรร์/ID)"
            ...
            Stats.Size = UDim2.new(0, 240, 1, 0)
            Stats.Visible = GridSettings.ShowStats
            Stats.TextXAlignment = Enum.TextXAlignment.Right
            Stats.Font = Enum.Font[SelectedTheme.TextFont] or Enum.Font.Gotham
            Stats.TextSize = 14
            Stats.TextColor3 = SelectedTheme.TextColor
            Stats.Text = ""
            ...
            Grid.CellPadding = GridSettings.CellPadding
            ...
            local rarityColor = {
                ...
                uncommon    = Color3.fromRGB(64,180,110),
                rare        = Color3.fromRGB(64,120,200),
                epic        = Color3.fromRGB(160,90,210),
                legendary   = Color3.fromRGB(225,165,45),
                mythical    = Color3.fromRGB(255,75,75),
            }

            -- เทมเพลตไอเท็ม
            local function createCell(item)
                -- ใช้ TextButton เพื่อรองรับ MouseButton1/2
                local Cell = Instance.new("TextButton")
                Cell.Name = tostring(item.id or item.name or "Item")
                Cell.AutoButtonColor = false
                Cell.Text = ""
                Cell.BackgroundColor3 = SelectedTheme.SecondaryElementBackground
                Cell.Size = UDim2.new(0, Grid.CellSize.X.Offset, 0, Grid.CellSize.Y.Offset)
                local corner = Instance.new("UICorner", Cell); corner.CornerRadius = UDim.new(0, 8)
                local stroke = Instance.new("UIStroke", Cell)
                stroke.Color = SelectedTheme.SecondaryElementStroke
                stroke.Thickness = 1

                -- เส้นสีตามแรร์ (ซ้อน stroke ชั้นใน)
                local rareBar = Instance.new("Frame")
                rareBar.Name = "Rarity"
                rareBar.Parent = Cell
                rareBar.Size = UDim2.new(1,0,0,3)
                rareBar.Position = UDim2.new(0,0,0,0)
                rareBar.BackgroundColor3 = rarityColor[string.lower(item.rarity or "common")] or rarityColor.common
                rareBar.BorderSizePixel = 0

                -- ไอคอน
                local Icon = Instance.new("ImageLabel")
                Icon.Parent = Cell
                Icon.BackgroundTransparency = 1
                Icon.Size = UDim2.new(0, Grid.CellSize.X.Offset-18, 0, Grid.CellSize.Y.Offset-30)
                Icon.Position = UDim2.new(0.5,0,0,10)
                Icon.AnchorPoint = Vector2.new(0.5,0)
                Icon.Image = typeof(item.icon)=="string" and item.icon or ("rbxassetid://"..tostring(item.icon or 0))
                ...
                Name.Position = UDim2.new(0, 8, 1, -18)
                Name.Size = UDim2.new(1, -16, 0, 14)
                Name.Font = Enum.Font[SelectedTheme.TextFont] or Enum.Font.Gotham
                Name.TextSize = 12
                Name.TextXAlignment = Enum.TextXAlignment.Center
                Name.TextColor3 = SelectedTheme.TextColor
                Name.Text = tostring(item.name or "Unknown")

                -- จำนวน (badge มุมขวาล่าง)
                ...
                Qty.Size = UDim2.new(0, 34, 0, 18)
                local qC = Instance.new("UICorner", Qty); qC.CornerRadius = UDim.new(0,6)
                Qty.Font = Enum.Font.GothamBold
                Qty.TextSize = 12
                Qty.TextColor3 = Color3.new(1,1,1)
                Qty.TextXAlignment = Enum.TextXAlignment.Center
                Qty.Text = "x"..tostring(item.qty or 1)

                -- Equipped tag (มุมซ้ายบน)
                if item.equipped then
                    local Tag = Instance.new("TextLabel")
                    Tag.Parent = Cell
                    Tag.BackgroundTransparency = 0.15
                    Tag.BackgroundColor3 = Color3.fromRGB(0,146,214)
                    Tag.Size = UDim2.new(0, 46, 0, 16)
                    Tag.Position = UDim2.new(0,4,0,4)
                    local tC = Instance.new("UICorner", Tag); tC.CornerRadius = UDim.new(0,4)
                ...
                Cell.MouseEnter:Connect(function()
                    ...
                    GridSettings.OnHover(item, true)
                end)
                Cell.MouseLeave:Connect(function()
                    TweenService:Create(Cell, TweenInfo.new(0.2, Enum.EasingStyle.Quint), {
                        BackgroundColor3 = SelectedTheme.SecondaryElementBackground
                    }):Play()
                    GridSettings.OnHover(item, false)
                end)
                Cell.MouseButton1Click:Connect(function()
                    pcall(function() GridSettings.OnClick(item) end)
                end)
                Cell.MouseButton2Click:Connect(function()
                    pcall(function() GridSettings.OnRightClick(item) end)
                end)
            ...
            -- ภายในสถานะ
            local state = {
                source = table.clone(GridSettings.Items),
                view   = {},
                filter = "",
            }

            local function applyFilterAndSort()
                -- filter (ชื่อ, แรร์, id, qty)
                local f = string.lower(state.filter or "")
                local tmp = {}
                for _, it in ipairs(state.source) do
                    local hay = (tostring(it.name or "") .. " " .. tostring(it.rarity or "") .. " " .. tostring(it.id or "") .. " " .. tostring(it.qty or "")):lower()
                    if f == "" or string.find(hay, f, 1, true) then
                        table.insert(tmp, it)
                    end
                end
                table.sort(tmp, GridSettings.Sorter)
                state.view = tmp
            end

            local function rebuild()
                Scroll:ClearAllChildren()
                Grid.Parent = Scroll -- keep grid
                for i, it in ipairs(state.view) do
                    local cell = createCell(it)
                    cell.LayoutOrder = i
                    cell.Parent = Scroll
                end
                -- stats
                if GridSettings.ShowStats then
                    local total = 0
                    for _, it in ipairs(state.view) do total += (it.qty or 1) end
                    Stats.Text = string.format("ชนิดไอเท็ม: %d  |  จำนวนรวม: %d", #state.view, total)
                end
            end

            local function softUpdateQty(id)
                for _, frm in ipairs(Scroll:GetChildren()) do
                    if frm:IsA("TextButton") and frm.Name == tostring(id) then
                        local it
                        for _, s in ipairs(state.source) do if tostring(s.id) == tostring(id) then it = s break end end
                        if it then
                            local qtyLabel = frm:FindFirstChild("Qty")
                            if qtyLabel and qtyLabel:IsA("TextLabel") then
                                qtyLabel.Text = "x"..tostring(it.qty or 1)
                            end
                        end
                        break
                    end
            ...
            Root.UIStroke.Transparency = 1
            Root.Title.TextTransparency = 1
            TweenService:Create(Root, TweenInfo.new(0.35, Enum.EasingStyle.Quint), {BackgroundTransparency = 0}):Play()
            TweenService:Create(Root.UIStroke, TweenInfo.new(0.35, Enum.EasingStyle.Quint), {Transparency = 0}):Play()
            TweenService:Create(Root.Title, TweenInfo.new(0.35, Enum.EasingStyle.Quint), {TextTransparency = 0}):Play()

            -- ค้นหา
            SearchBox:GetPropertyChangedSignal("Text"):Connect(function()
                state.filter = SearchBox.Text
                applyFilterAndSort()
                rebuild()
            end)
            ...
            function API:SetItems(items)
                state.source = table.clone(items or {})
                applyFilterAndSort()
                rebuild()
            end

            function API:AddItem(item)
                table.insert(state.source, item)
                applyFilterAndSort()
                rebuild()
            end

            function API:RemoveItem(id)
                for i = #state.source,1,-1 do
                    if tostring(state.source[i].id) == tostring(id) then
                        table.remove(state.source, i)
                    end
                end
                applyFilterAndSort()
                rebuild()
            ...
            function API:UpdateItem(id, patch)
                for _, it in ipairs(state.source) do
                    if tostring(it.id) == tostring(id) then
                        for k,v in pairs(patch or {}) do it[k] = v end
                        break
                    end
                end
                applyFilterAndSort()
            ...
            function API:UpdateQty(id, qty)
                for _, it in ipairs(state.source) do
                    if tostring(it.id) == tostring(id) then
                        it.qty = math.max(0, tonumber(qty) or 0)
                        break
                    end
                end
                applyFilterAndSort()
                softUpdateQty(id) -- เร็ว ไม่ต้อง rebuild ทั้งกริด
            end

            function API:SetFilter(text)
                state.filter = tostring(text or "")
                applyFilterAndSort()
                rebuild()
            end

            function API:SortWith(compareFn)
                GridSettings.Sorter = compareFn or GridSettings.Sorter
                applyFilterAndSort()
                rebuild()
            end
...
function RayfieldLibrary:LoadConfiguration()
	if CEnabled then
		pcall(function()
			if CFileName and isfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension) then
